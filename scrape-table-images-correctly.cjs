...
async function scrapeTableImages(url, sku) {
  try {
    ...
    $('.product-item-detail-slider-container .product-item-detail-slider-slides-container .product-item-detail-slider-slide img').each((i, elem) => {
    ...
    if (images.length === 0) {
      $('img[itemprop="image"]').each((i, elem) => {
        const src = $(elem).attr('src');
        if (src && src.startsWith('https://')) {
          images.push(src);
        }
    ...
    console.log(`Found ${images.length} images for ${sku}`);
...
async function scrapeAll() {
  ...
  for (const table of tables) {
    const result = await scrapeTableImages(table.url, table.sku);
    results.push(result);
