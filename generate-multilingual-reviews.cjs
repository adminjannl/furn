...
const reviewData = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'multilingual-natural-reviews.json'), 'utf8')
);
...
async function generateReviews() {
  ...
  console.log('Generating 463 multilingual natural reviews...\n');
  ...
  for (const [rating, count] of Object.entries(distribution)) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const templates = reviewData[language][rating];
  ...
  const langCounts = languages.reduce((acc, lang) => {
    acc[lang] = reviews.filter(r => r.language === lang).length;
    return acc;
  }, {});
  ...
  console.log(`\n✅ Done! Generated ${reviews.length} multilingual reviews with ${avgRating.toFixed(2)} average rating`);
}
